...
const Modal = ({ entry, onSave, onClose, saving }) => {
  const [form, setForm] = useState(entry ? { ...entry } : { insight: "", tip: "", isActive: true });
  const f = (key) => (e) => setForm({ ...form, [key]: e.target.value });
  const valid = form.insight.trim() !== "";
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="bg-[#00bf62] px-5 py-3 flex justify-between items-center">
          <h2 className="text-base font-bold text-white flex items-center gap-2">
            <MdFavorite /> {entry ? "Edit Insight" : "Add Insight"}
          </h2>
        ...
        <div className="px-5 py-4 space-y-3">
          <div>
            <label className="block text-xs font-bold text-gray-700 mb-1">Insight <span className="text-red-500">*</span></label>
            <textarea rows={3} className={`${inp} resize-none`} placeholder="Enter a parenting insight..." value={form.insight} onChange={f("insight")} />
          </div>
          <div>
            <label className="block text-xs font-bold text-gray-700 mb-1">Tip (optional)</label>
            <textarea rows={2} className={`${inp} resize-none`} placeholder="Add a practical tip..." value={form.tip} onChange={f("tip")} />
          </div>
...
const ViewModal = ({ item, onClose }) => (
  <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 px-4">
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
      <div className="bg-[#00bf62] px-5 py-3 flex justify-between items-center">
        <h2 className="text-base font-bold text-white flex items-center gap-2"><MdFavorite /> View Insight</h2>
        <button onClick={onClose} className="text-white/80 hover:bg-white/20 rounded-full w-8 h-8 flex items-center justify-center transition"><MdClose /></button>
      </div>
      <div className="px-5 py-4 space-y-3">
        <div><p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-1">Insight</p><p className="text-sm text-gray-800">{item.insight}</p></div>
        {item.tip && <div><p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-1">Tip</p><p className="text-sm text-gray-600">{item.tip}</p></div>}
      </div>
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Parenting Insights</h1>
            <p className="text-gray-500 text-xs mt-0.5">Manage parenting insights shown on the home screen</p>
          </div>
        </div>
        <button onClick={() => { setEditItem(null); setModalOpen(true); }}
          ...
          <MdAdd className="text-lg" /> Add Insight
        </button>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-[#00bf62] text-white">
              ...
              <th className="px-5 py-3.5 text-left font-semibold">Insight</th>
              <th className="px-5 py-3.5 text-left font-semibold">Tip</th>
              ...
              <tr key={item._id} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                ...
                <td className="px-5 py-3.5">
                  <button
                    onClick={async () => {
                      try {
                        await api.parentingInsights.update(item._id, { ...item, isActive: !item.isActive });
                        await load();
